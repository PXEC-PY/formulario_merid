...
// stripped (e.g. "4.123.456" or "4123456"). Kept intentionally permissive — this is a
// citizen-facing form, not a validity check against the Registro Civil.
export const ciSchema = z
  .string()
  .transform((v) => v.replace(/[.\s]/g, ""))
  .pipe(z.string().regex(/^\d{5,9}$/, "Ingrese un número de C.I. válido"));

// Paraguayan RUC: digits, optional hyphen and one check digit (e.g. "80012345-6").
export const rucSchema = z
  .string()
  .regex(/^\d{4,9}-?\d$/, "Ingrese un RUC válido (ej: 80012345-6)");
